...
class OllamaManager:
    ...
    def list_models(self):
        try:
            # 添加creationflags参数以在后台运行命令，避免终端窗口闪烁
            if os.name == 'nt':  # Windows
                result = subprocess.run(['ollama', 'list'], capture_output=True, text=True, check=True, encoding='utf-8', creationflags=subprocess.CREATE_NO_WINDOW)
            else:  # Linux/Mac
    ...
    def get_model_status(self, model_name):
        ...
        try:
            ...
            # 检查模型是否在运行
            if os.name == 'nt':  # Windows
                result = subprocess.run(['ollama', 'ps'], capture_output=True, text=True, encoding='utf-8', creationflags=subprocess.CREATE_NO_WINDOW)
            else:  # Linux/Mac
    ...
    def stop_model(self, model_name):
        try:
            ...
            # 停止运行的模型
            if os.name == 'nt':  # Windows
                result = subprocess.run(['ollama', 'stop', resolved_model_name], capture_output=True, text=True, check=True, encoding='utf-8', creationflags=subprocess.CREATE_NO_WINDOW)
            else:  # Linux/Mac
    ...
    def remove_model(self, model_name):
        try:
            ...
            resolved_model_name = self.resolve_model_name(model_name)
            if os.name == 'nt':  # Windows
                result = subprocess.run(['ollama', 'rm', resolved_model_name], capture_output=True, text=True, check=True, encoding='utf-8', creationflags=subprocess.CREATE_NO_WINDOW)
            else:  # Linux/Mac
